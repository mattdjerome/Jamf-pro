#!/bin/bash
# update_policy_self_service_category.sh
# For each printer name in the CSV:
#   Finds a policy named "Wincraft - <printer name>" and sets its Self Service category to "Wincraft"
#
# CSV format: one printer name per line, no header
# Usage: ./script.sh CLIENT_ID CLIENT_SECRET /path/to/printers.csv https://jamf.example.com

# ── Configuration ────────────────────────────────────────────────────────────
JAMF_URL="$4"           # No trailing slash
CLIENT_ID="$1"
CLIENT_SECRET="$2"
CSV_FILE="$3"           # Path to your CSV file
SS_CATEGORY="$5"  # Self Service category to apply to all policies
...
while IFS= read -r printer_name || [[ -n "$printer_name" ]]; do
    ...
    # ── Update Matching Policy Self Service Category ────────────────────────
...
<policy>
    <self_service>
        ...
        if [[ "$http_code" == "201" ]]; then
            echo "  [Policy]  ✓ Self Service category set to '${SS_CATEGORY}' (ID: ${policy_id}, HTTP ${http_code})"
            ((policy_success++))
        else
            echo "  [Policy]  ✗ FAILED to update Self Service category (ID: ${policy_id}, HTTP ${http_code})"
            ((policy_failed++))
        fi
    fi
